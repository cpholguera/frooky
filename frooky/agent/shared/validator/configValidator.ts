...
export function validateFrookyConfig(
	frookyConfig: FrookyConfig,
	platform: Platform,
): HookValidatorResult {
	...
	if (frookyConfig.metadata) {
		frooky.log.info(
			`frooky config metadata:\n${JSON.stringify(frookyConfig.metadata, null, 2)}`,
		);
		if (
			frookyConfig.metadata.platform?.toLowerCase() !==
		...
		frooky.log.warn(
			"This frooky configuration does not have metadata. Consider adding them for better results.",
		);
	}
	...
	if (frookyConfig.hooks) {
		hookValidatorResult = validateHooks(
			frookyConfig.hooks,
			platform,
			frookyConfig.metadata,
		);
