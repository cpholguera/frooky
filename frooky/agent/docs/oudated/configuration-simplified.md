...
If you only want to hook a certain overload, specify it by adding one or more `overload` to the `<java_method>`:
...
> methods:
>   - "$init()"
>   - "$init(action: String!, uri: Uri!)"
>   - "putExtra(name: String!, value: String?)"
>   - "putExtra(name: String!, value: Array<CharSequence!>?)"
>  ```
>
...
> Intent.putExtra(name: String!, value: Array<CharSequence!>?): Intent
> ```
...
>  ```
>
> Frooky will now try to decode the arguments and the return value based the type. This `<hook_configuration>` will hook the following Objective-C method:
>
> ```objectivec
> + (NSURLSession *) sessionWithConfiguration:(NSURLSessionConfiguration *) configuration;
> ```
